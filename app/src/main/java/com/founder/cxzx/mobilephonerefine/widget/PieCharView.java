...
//        }
    ...
    private void paintPie(final Canvas mCanvas) {
        pieAngles = new float[kindsMap.size()];
        ...
        if (kindsMap.size() > 0) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    float x = event.getX() - ((width - getPaddingLeft() - getPaddingRight()) / 2);
                    float y = event.getY() - ((height - getPaddingBottom() - getPaddingTop()) / 2);
                    float touchAngle = 0;
                    if (x < 0 && y < 0) {
                        touchAngle += 180;
                    } else if (y < 0 && x > 0) {
                    ...
                    touchAngle = touchAngle - 0.0f;
                    if (touchAngle < 0) {
                        touchAngle = touchAngle + 360;
                    }
                    float touchRadius = (float) Math.sqrt(y * y + x * x);
                    if (mRadius / 2 + dp2px(10) < touchRadius && touchRadius < mRadius) {
                        angleId = -Arrays.binarySearch(pieAngles, (touchAngle)) - 1;
                        Log.e("PieCharView", "angleId:" + angleId);
                        invalidate();
                    }
                    return true;
